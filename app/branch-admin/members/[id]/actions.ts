...
async function ensureMemberWriteAccess(memberId: string) {
  ...
  if (role === "branch_admin" && profile.branch_id !== member.branch_id) {
    throw new Error("You cannot edit members outside your branch.");
  }
...
export async function upsertFamilyDetails(memberId: string, formData: FormData): Promise<ActionResult> {
  try {
    ...
    revalidatePath(`/branch-admin/members/${memberId}`);

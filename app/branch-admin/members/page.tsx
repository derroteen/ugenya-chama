...
export default async function BranchMembersPage({ searchParams }: PageProps) {
  ...
  const errorCode = getParamValue(resolvedParams.error).trim();
  ...
  const { data: branches } = canChooseBranch
    ? await supabase.from("branches").select("id, name, code").order("name", { ascending: true })
    : { data: [] as BranchOption[] };

  const branchOptions = branches ?? [];
  const selectedBranch = canChooseBranch
    ? branchOptions.find((branch) => branch.code.toUpperCase() === branchParam)
    : null;
  ...
  if (role === "branch_admin") {
    if (!ownBranchId) {
      membersQuery = membersQuery.eq("branch_id", "00000000-0000-0000-0000-000000000000");
    } else {
      membersQuery = membersQuery.eq("branch_id", ownBranchId);
    }
      ...
      <section className="mx-auto w-full max-w-6xl rounded-2xl border border-slate-200 bg-white px-6 py-8 shadow-sm sm:px-8 sm:py-10 lg:px-10 lg:py-12">
        ...
        <form method="get" className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className={canChooseBranch ? "sm:col-span-1" : "sm:col-span-2"}>
            ...
            <div className="flex items-end">
              <button
                type="submit"
                className="inline-flex w-full items-center justify-center rounded-lg bg-[#1d3a8a] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#16306f] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1d3a8a] focus-visible:ring-offset-2"
              >
                Apply Filters
              </button>
            </div>
        ...
        {errorCode === "member_not_found_or_forbidden" ? (
          <div
            role="alert"
            className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 sm:text-base"
          >
            That member record was not found or you do not have permission to view it.
          </div>
        ) : null}
                    ...
                    <tr key={member.id} className="hover:bg-slate-50/70">
                      ...
                      {role !== "branch_admin" ? (
                        <td className="px-4 py-4 text-sm text-slate-700 sm:px-6">
                          {getJoinedBranchName(member.branches) ?? "-"}
                        </td>
                      ) : null}
